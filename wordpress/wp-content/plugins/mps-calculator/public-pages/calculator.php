...
if( $type === 'our' )
{
	$countries_list_index = array_values( $countries_list );
?>
	<div class="calculate border-radius-10px bsizing-border-box float-left">
		<h1 class="calculate-headline float-left"><?php echo $translate_locations_ua_reverse[ 'shipping_cost_calculator' ]; ?></h1>
				...
				<section class="calculate-section float-left">
					...
					<div class="location">
						<select name="sender_location_country" class="location-country float-left font-size-d8 border-radius-5px shadow-light">
							...
							foreach( $countries_list_index[0] as $country )
							{
								
							?>
								<option value="<?php echo $country[ 'country' ]; ?>"><?php echo $translate_locations_ua_reverse[ $country[ 'country' ] ]; ?></option>
							<?php
					...
					<div class="coption-select-container coption-grey-border float-left bsizing-border-box">
						...
						<select name="city" class="coption-select">
							...
							<?php
								...
								foreach( $countries_list as $city )
								{
							?>
...
?>
	...
	<script type="text/template" id="calculator-order-template">
		<section class="calculate-data-item float-left bsizing-border-box" id="delivery-type-{{DELIVERY_TYPE}}">
			<h3 class="calculate-order-headline">{{DELIVERY_TYPE_TITLE}}</h3>
						...
						else
						{
							...
							function getLocationOption( deliveryCurrentType, locationType, selectedValue )
							{
								...
								for( let c in countries_list[ deliveryCurrentType.delivery_type ] )
								{
									...
									if( registerLocations.indexOf( currentLocation.city ) !== -1 )
									{
										continue;
									}
									...
									{
										LOCATION_SLUG: currentLocation[ locationType ],
										LOCATION_NAME: translateLocationsReverseUA[ currentLocation[ locationType ] ],
										SELECTED: currentLocation[ locationType ] === selectedValue ? 'selected' : ''
									};

									locationItemsHtml += templateCompilator.compile( locationItem.html(), templateLocationParams );

									registerLocations.push( currentLocation.country );
								}
							...
							for( let i in deliveryTypes )
							{
								...
								console.log( translateLocationsReverseUA[ deliveryCurrentType.delivery_type ] );
								...
								templateOrderParams =
								{
									...
									ORDER_SUBMIT_BUTTON_TITLE: price.status === 'undetermined' ? 'Швидкий звязок' : translateLocationsReverseUA[ 'order_submit_button' ],
		...
		calculateParamsButton.on( 'click', function()
		{
			...
			oValidator.validateFields( function()
			{
				...
				if( ! checkErrors )
				{
					...
					let calculateData = JSON.parse( '<?php echo json_encode( $countries_list ); ?>' );
